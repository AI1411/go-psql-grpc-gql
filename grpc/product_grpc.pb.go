...
func (c *productServiceClient) ChangeStatus(ctx context.Context, in *ChangeProductStatusRequest, opts ...grpc.CallOption) (*ChangeProductStatusResponse, error) {
	out := new(ChangeProductStatusResponse)
	err := c.cc.Invoke(ctx, "/product.v1.ProductService/ChangeStatus", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _ProductService_ChangeStatus_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ChangeProductStatusRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(ProductServiceServer).ChangeStatus(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/product.v1.ProductService/ChangeStatus",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ProductServiceServer).ChangeStatus(ctx, req.(*ChangeProductStatusRequest))
	}
	return interceptor(ctx, in, info, handler)
}
